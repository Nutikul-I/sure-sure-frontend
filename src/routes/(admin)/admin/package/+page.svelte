<script lang="ts">
  ...
  export let listPackages: PackageData[] = [];
  let search = '';
  ...
  // เรียกใช้ฟังก์ชัน GetPackage เมื่อหน้าโหลด
  onMount(async () => {
    await GetPackage();
  });

  // ฟังก์ชันค้นหาข้อมูลแพ็กเกจ
  ...
  // ฟังก์ชันสำหรับสร้าง/อัปเดตแพ็กเกจ
        ...
        {:else}
          {#each listPackages as item, index}
            <tr class="text-start">
              ...
              <td class="text-end">{item.amount === 0 ? 'Unlimited' : item.amount.toLocaleString()}</td>
              <td class="text-center">{item.duration}</td>
              <td class="text-center"><small class="px-2 rounded-pill {item.is_active ? 'badge-success' : 'badge-danger'}">{item.is_active ? 'ACTIVE' : 'DEACTIVE'}</small></td>
              <td class="p-1 sm:p-2">
                <svg
                  on:click={() => {
                    id = item.id;
...
<dialog id="create-update" class="modal px-0">
  <div class="modal-box bg-white w-11/12 max-w-md p-3">
    ...
    <label class="label">
      ...
      <input type="text" class="form-control" bind:value={package_name} maxlength="250" />
    </label>
    <label class="label">
      <span class="label-text text-black w-2/5">ราคา:</span>
      <input type="number" class="form-control" bind:value={package_price} />
    </label>
    <label class="label">
      <span class="label-text text-black w-2/5">โควต้าสูงสุด:</span>
      <input type="number" class="form-control" bind:value={quota_limit} />
    </label>
    <label class="label">
    ...
    <label class="label">
      ...
      <input type="number" class="form-control" bind:value={duration} />
    </label>
